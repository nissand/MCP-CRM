...
| `functions/` | CRM business logic, one module per entity. Every public function authenticates via `getAuthContext` and scopes all data access to the caller's tenant |
| `lib/` | Shared helpers: auth context resolution (`utils.ts`, `tokenAuth.ts`), typed errors (`errors.ts`), Zod validators (`validators.ts`) |
